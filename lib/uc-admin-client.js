...
/**
  Creates a new UidCloud Admin client
  @param {object} settings - an object containing the settings
  @param {string} settings.baseUrl - The baseurl for the UidCloud server - ex: http://localhost:8080/auth,
  @param {string} settings.username - The username to login to the uidcloud server - ex: admin
  @param {string} settings.password - The password to login to the uidcloud server - ex: *****
  @param {string} settings.grant_type - the type of authentication mechanism - ex: password,
  @param {string} settings.client_id - the id of the client that is registered with UidCloud to connect to - ex: admin-cli
  ...
  const adminClient = require('uidcloud-admin-client')
  const settings = {
 ...
 */
function uidcloudAdminClient (settings) {
  settings = settings || {};
...
module.exports = uidcloudAdminClient;
